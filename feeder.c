...
bool feeded_today;
bool demo_mode;
bool ready, done, feeding, display_off, display_off_tobe;
int counter_on_lcd; 
...
int main(int argc, char *argv[]) 
{
	...
	if (adjustments > 0) {
		printf("Adjusting for %d steps counter clockwise...\n", adjustments);
		for (int i = 0; i < adjustments; i++) {
			rotate(pins, COUNTER_CLOCKWISE);
		}
	}
	else if (adjustments < 0) {
		printf("Adjusting for %d steps clockwise...\n", 0-adjustments);
		for (int i = 0; i < (0 - adjustments); ++i) {
			rotate(pins, CLOCKWISE);
		}
	}
	else {
		ready = feeding = display_off = display_off_tobe = false;
		counter_on_lcd = 0;
		...
		while (!done) {
			...
			for (int i = 0; i < (demo_mode ? 4 : 60); ++i) {
				if (done)
					break;
				delayMS(1000);
			}
